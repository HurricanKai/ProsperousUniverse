...
namespace SocketIOClient
{
    ...
    public class SocketIO : IDisposable
    {
        ...
        public SocketIO(ILoggerFactory loggerFactory, string uri) : this(loggerFactory, new Uri(uri)) { }

        /// <summary>
        /// Create SocketIO object with options
        /// </summary>
        /// <param name="uri"></param>
        public SocketIO(ILoggerFactory loggerFactory, Uri uri) : this(loggerFactory, uri, new SocketIOOptions()) { }
        ...
        public SocketIO(ILoggerFactory loggerFactory, string uri, SocketIOOptions options) : this(loggerFactory, new Uri(uri), options) { }
        ...
        /// <param name="uri"></param>
        /// <param name="options"></param>
        public SocketIO(ILoggerFactory loggerFactory, Uri uri, SocketIOOptions options)
        {
            _loggerFactory = loggerFactory;
            _logger = _loggerFactory.CreateLogger<SocketIO>();
        ...
        List<OnAnyFallbackHandler> _onAnyFallbackHandlers;
        ...
        private void Initialize()
        {
            ...
            _onAnyFallbackHandlers = new List<OnAnyFallbackHandler>();

            JsonSerializer = new SystemTextJsonSerializer();
            _connectionTokenSource = new CancellationTokenSource();
            HttpClient = new HttpClient();
            ClientWebSocketProvider = () => new DefaultClientWebSocket(null);
        ...
        private void CreateRouterIfNull()
        {
            if (Router == null)
            {
                ...
                {
                    Router = new HttpRouter(_loggerFactory.CreateLogger<HttpRouter>(), HttpClient, ClientWebSocketProvider, Options);
                }
                else
                {
                    Router = new WebSocketRouter(_loggerFactory, HttpClient, ClientWebSocketProvider, Options);
                }
        ...
        private void EventMessageHandler(EventMessage m)
        {
            ...
            foreach (var item in _onAnyHandlers)
            {
                ...
                {
                    _logger.LogDebug(e, "Exception in OnAny");
                }
            }
            if (_eventHandlers.ContainsKey(m.Event))
            {
                ...
                {
                    _logger.LogDebug(e, "Exception in On");
                }
            }
            else
            {
                foreach (var item in _onAnyFallbackHandlers)
                {
                    try
                    {
                        item(m.Event, res);
                    ...
                    {
                        _logger.LogDebug(e, "Exception in OnAnyFallback");
                    }
                }
            }
        }
        ...
        private void BinaryMessageHandler(BinaryMessage msg)
        {
            if (_eventHandlers.ContainsKey(msg.Event))
            {
                ...
                {
                    _logger.LogDebug(e, "Exception in On Binary");
                }
            }
        }
        ...
        private void BinaryAckMessageHandler(ClientBinaryAckMessage msg)
        {
            if (_ackHandlers.ContainsKey(msg.Id))
            {
                ...
                {
                    _logger.LogDebug(e, "Exception in On BinaryAck");
                }
            }
        }
        ...
        private void OnMessageReceived(IMessage msg)
        {
            ...
            {
                _logger.LogDebug(e, "Exception in OnMessageReceived");
            }
        }
        ...
        public async Task DisconnectAsync()
        {
            if (Connected)
            {
                ...
                {
                    _logger.LogDebug(e, "Exception in Disconnect");
                }
        ...
        public void OnFallbackAny(OnAnyFallbackHandler handler)
        {
            if (handler != null)
            {
                _onAnyFallbackHandlers.Add(handler);
            }
        }
        ...
        private async void InvokeDisconnect(string reason)
        {
            if (Connected)
            {
                ...
                {
                    _logger.LogDebug(e, "Exception in Invoke Disconnect");
                }
