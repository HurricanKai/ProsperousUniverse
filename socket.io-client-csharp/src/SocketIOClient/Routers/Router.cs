...
namespace SocketIOClient.Routers
{
    public abstract class Router : IDisposable
    {
        ...
        public Router(ILogger logger, HttpClient httpClient, Func<IClientWebSocket> clientWebSocketProvider, SocketIOOptions options)
        {
            _logger = logger;
            ...
            if (msg != null)
            {
                ...
                else
                {
                    ...
                    if (msg.Type == MessageType.Ping)
                    {
                        ...
                        {
                            _logger.LogDebug(e, "Exception in Ping");
                            OnTransportClosed();
                        }
                    }
                }
        ...
        {
            _logger.LogDebug($"[Websocket] Aborted, " + e.Message);
            OnTransportClosed();
        }
